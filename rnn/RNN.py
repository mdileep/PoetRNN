...
#static methods in this class seemed to make testing easier.  This class is
#largely taken from Andrej Karpathy's
...
class LSTM:
    ...
    #LSTM forward pass with optional dropout, the drop_mask feature is useful
    #for testing, but largely unnecessary
    @staticmethod
    def batch_forward(X,model,output_layer=True,c0=None,h0=None,drop_prob=0.0,drop_mask=None):
        ...
        if output_layer:
            Wout = model['Wout']
        # X is a (n,b,v) array where n=sequence length, b=batch size and
        # v=vocab size
        ...
        Houtstack = np.ones((n,b,int(d+1))) #hidden representation with bias
        IFOG = np.zeros((n, b, int(d * 4))) # input, forget, output, gate (IFOG)
        IFOGf = np.zeros((n, b,  int(d * 4))) # after nonlinearity
        ...
        for t in range(n):
            ...
            IFOGf[t,:,:3 * int( d)] = 1.0 / (1.0 + np.exp(-IFOG[t,:,:3 *int( d)]))
            IFOGf[t,:,3 * int( d):] = np.tanh(IFOG[t,:,3 *int( d):])
            if t == 0:
                prevc = c0
            else:
                prevc = C[t - 1]
            C[t] = prevc * IFOGf[t,:,int( d):2 * int( d)] + IFOGf[t,:,:int( d)] * IFOGf[t,:,3 * int( d):]
            Ct[t] = np.tanh(C[t])
            Hout[t] = IFOGf[t,:,2 * int( d):3 * int( d)] * Ct[t]
        if drop_prob > 0:
...
def LSTM_sample(model,temp=1.0,seed=None,max_length=1000,num_layers=2):
    ...
    c1 = np.zeros((1,int(d1)))
    c2 = np.zeros((1,int(d2)))
